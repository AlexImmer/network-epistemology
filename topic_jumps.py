...
def obtain_jump_edge_distances(X_topic, years, max_mem=10**7, tradition=True):
    ...
    for year in tqdm(sorted(list(years))[1:]):
        ...
        for res in res_list:
            ixs.extend(res[0])
            dists.extend(res[1])
        res = pd.DataFrame(index=ixs)
        res['topic_' + col] = dists
...
def find_edges(X_prev):
    ...
    map = {}
    for s, e in zip(X_prev.index[starts], X_cur.index[ends]):
        if map.get(s):
            map[s].append(e)
        else:
            map[s] = [e]
    return list(map.keys()), [g_dists.loc[map[x], col].min() for x in map]
